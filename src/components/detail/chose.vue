  ...
  <section class="chose" v-if="view">
    <div class="chose-view">
      <h1 class="chose-view-title">
        {{view.title}} ~~
        <span>({{colText}} - {{sizeText}})</span><br>
        <span>(in shopping car)</span>
      </h1>
...
export default {
  ...
  computed: mapState({
    ...
    // return value of (innerText)
    colText() {
      return this.view.chose[this.colSelected].col
    },
    // return the value of (innerText)
    sizeText() {
  ...
  methods: {
    ...
    //value for colSelected,and make judgement (active)
    colChose(i) {
