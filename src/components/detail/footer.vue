<template lang="html">
  <footer class="footer">
    <router-link :to="{name:'home'}" class="footer-index">
      <i class="icon-index"></i>
    </router-link>
    <router-link  :to="{name:'shopPage'}" class="footer-gocar">
      <i class="icon-car"></i>
...
export default {
  computed: {
    ...
    count () {
      //after page refreash and data will deleted 
  ...
  methods: {
    addIntoCar () {
      ...
      MessageBox
        .confirm
        (
        `Title:${product[0].title}</br>` +
        `Price:${product[0].price}</br>` +
        `Size:${product[0].size}</br>` +
        `Color:${product[0].col}</br>` +
        `ID:${product[0].id}</br>`
        )
        .then(action => {      //click successful and run the function
